...
<svelte:head><title>{t('privacy-policy')} - {appName}</title></svelte:head>

<div id="pageContent" class="elementToPadMenu">
    <div class="container">
        <h1 style="margin-top: 0; padding-top: 0">{@html t('privacy-policy')}</h1>
        <p>{@html t('privacy-policy-last-edit')}</p>
        <hr />
        {@html t('privacy-policy-text')}
    </div>
